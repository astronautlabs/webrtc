...
import { getUserMedia, MediaStream, RTCPeerConnection } from '..';
...
function testSettingEnabled(stream: MediaStream) {
...
function getRemoteMediaStream() {
    ...
    var trackEventPromise = new Promise<RTCTrackEvent>(resolve => pc.ontrack = resolve);
    return pc.setRemoteDescription(offer)
        .then(() => trackEventPromise)
        .then(trackEvent => {
            pc.close();
