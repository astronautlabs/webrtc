...
namespace node_webrtc {
    /**
     * Provides a mechanism to send events from any thread to the libuv event loop
     * thread and have them be dispatched in that context. Uses the libuv Async
     * Handle mechanism to do this- when Dispatch() is called on this object,
     * uv_async_send is called to signal libuv to run our registered callback. Run()
     * then runs on the libuv event loop thread and dispatches any events that have
     * been enqueued before returning control back to the event loop.
     */
    template <typename T>
