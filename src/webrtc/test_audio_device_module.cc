...
namespace node_webrtc {
    ...
    namespace {
        ...
        public:
            ...
            TestAudioDeviceModuleImpl(std::unique_ptr<Capturer> capturer, std::unique_ptr<Renderer> renderer, float speed = 1) :
            ...
            bool WaitForPlayoutEnd(
                webrtc::TimeDelta timeout_ms = webrtc::Event::kForever
            ) override {
                        ...
                        if (rendering_) {
                            ...
                            if (audio_callback_) {
                                audio_callback_->NeedMorePlayData(
                                    SamplesPerFrame(sampling_frequency), 2, renderer_->NumChannels(), sampling_frequency, playout_buffer_.data(), samples_out, &elapsed_time_ms, &ntp_time_ms
                                );
                            }
                            const bool keep_rendering = renderer_->Render(webrtc::ArrayView<const int16_t>(playout_buffer_.data(), samples_out));
                            if (!keep_rendering) {
                                rendering_ = false;
                                done_rendering_.Set();
                            }
        ...
        public:
            ...
            PulsedNoiseCapturerImpl(int16_t max_amplitude, int sampling_frequency_in_hz, int num_channels) :
            ...
            bool Capture(webrtc::BufferT<int16_t>* buffer) override {
                ...
                buffer->SetData(
                    TestAudioDeviceModule::SamplesPerFrame(sampling_frequency_in_hz_) * num_channels_,
                    [&](auto data) {
                    if (fill_with_zero_) {
        ...
        public:
            WavFileReader(std::string filename, int sampling_frequency_in_hz, int num_channels) :
                WavFileReader(absl::make_unique<webrtc::WavReader>(filename), sampling_frequency_in_hz, num_channels) { }
        ...
        private:
            WavFileReader(std::unique_ptr<webrtc::WavReader> wav_reader, int sampling_frequency_in_hz, int num_channels) :
                sampling_frequency_in_hz_(sampling_frequency_in_hz),
                num_channels_(num_channels),
        ...
        public:
            WavFileWriter(std::string filename, int sampling_frequency_in_hz, int num_channels) :
                WavFileWriter(absl::make_unique<webrtc::WavWriter>(filename, sampling_frequency_in_hz, num_channels), sampling_frequency_in_hz, num_channels) { }
        ...
        private:
            WavFileWriter(std::unique_ptr<webrtc::WavWriter> wav_writer, int sampling_frequency_in_hz, int num_channels) :
                sampling_frequency_in_hz_(sampling_frequency_in_hz),
        ...
        public:
            BoundedWavFileWriter(const std::string& filename, int sampling_frequency_in_hz, int num_channels) :
                sampling_frequency_in_hz_(sampling_frequency_in_hz),
                wav_writer_(filename, sampling_frequency_in_hz, num_channels),
                num_channels_(num_channels),
                silent_audio_(
                    TestAudioDeviceModule::SamplesPerFrame(sampling_frequency_in_hz) * num_channels,
                    0
                ) { }
                ...
                if (started_writing_) {
                    ...
                    if (begin < end) {
                        ...
                        while (trailing_zeros_ > 0) {
                            const size_t zeros_to_write = std::min(trailing_zeros_, silent_audio_.size());
    ...
    TestAudioDeviceModule::CreateTestAudioDeviceModule(
        std::unique_ptr<Capturer> capturer, std::unique_ptr<Renderer> renderer, float speed
    ) {
    ...
    TestAudioDeviceModule::CreatePulsedNoiseCapturer(int16_t max_amplitude, int sampling_frequency_in_hz, int num_channels) {
        return std::unique_ptr<TestAudioDeviceModule::PulsedNoiseCapturer>(
            new PulsedNoiseCapturerImpl(max_amplitude, sampling_frequency_in_hz, num_channels)
        );
    }

    std::unique_ptr<TestAudioDeviceModule::Renderer>
    TestAudioDeviceModule::CreateDiscardRenderer(int sampling_frequency_in_hz, int num_channels) {
    ...
    TestAudioDeviceModule::CreateWavFileReader(std::string filename, int sampling_frequency_in_hz, int num_channels) {
    ...
    std::unique_ptr<TestAudioDeviceModule::Renderer>
    TestAudioDeviceModule::CreateWavFileWriter(std::string filename, int sampling_frequency_in_hz, int num_channels) {
    ...
    std::unique_ptr<TestAudioDeviceModule::Renderer>
    TestAudioDeviceModule::CreateBoundedWavFileWriter(std::string filename, int sampling_frequency_in_hz, int num_channels) {
        return std::unique_ptr<TestAudioDeviceModule::Renderer>(
            new BoundedWavFileWriter(filename, sampling_frequency_in_hz, num_channels)
        );
    }
